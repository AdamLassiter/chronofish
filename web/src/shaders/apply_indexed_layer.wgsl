...
fn apply_layer_naive(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  for (var sample = 0u; sample < params.sample_count; sample = sample + 1u) {
    ...
    let feature = select(1.0, features[dataset_sample * params.input_size + input_index], input_index < params.input_size);
    gradient = gradient + deltas[sample * params.output_size + output_index] * feature;
  }
...
fn apply_layer(
  ...
  for (var base = 0u; base < params.sample_count; base = base + 16u) {
    ...
    let dataset_sample = select(0u, batch_indices[feature_sample], feature_sample < params.sample_count);
    feature_tile[local.y * 16u + local.x] = select(
      0.0,
      select(1.0, features[dataset_sample * params.input_size + input_index], input_index < params.input_size),
