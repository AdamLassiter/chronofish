...
test("browser CPU finalists use paired matches on a common score scale", async () => {
  ...
  assert.match(worker, /for \(const candidateColor of \[game\.turn, engineOppositeColor\(game\.turn\)\]\)/);
...
test("browser CPU training performs bounded evolutionary generations", async () => {
  ...
  assert.match(worker, /const improved = cpuTrainingCandidateImproved\(winner\?\.score, baselineScore, bestCandidate\?\.score\)/);
