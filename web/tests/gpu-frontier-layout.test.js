...
test("GPU snapshot wire codes have engine-owned counterparts", async () => {
  ...
  assert.match(worker, /engine\.chronofish_gpu_turn_status_json_bytes\(ptr, byteLength\)/);
  ...
  assert.match(worker, /const meta = engine \? candidateInputMetaFromEngine\(words, engine\) :/);
...
test("GPU frontier publishes diagnostics needed for rollout gates", async () => {
  ...
  assert.match(worker, /engine\.chronofish_gpu_turn_completion_max_moves/);
...
test("GPU training harness uses bounded parallel WebGPU workers", async () => {
  ...
  assert.match(worker, /gpuRolloutMaxPlies\(0, 0\)/);
...
test("GPU training rollouts apply complete returned turns", async () => {
  ...
  assert.match(worker, /loadTrainingSnapshot\(engine, current\)/);
  assert.match(worker, /engine\.chronofish_apply_move\(/);
  assert.match(worker, /engine\.chronofish_snapshot_json\(\)/);
  ...
  assert.doesNotMatch(worker.slice(worker.indexOf("async function applyWorkerTurn"), worker.indexOf("async function applyCpuWorkerTurn")), /type: "applyMove"[\s\S]*type: "submitTurn"/);
