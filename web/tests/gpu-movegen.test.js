...
test("legacy GPU move generation carries en-passant board metadata", async () => {
  ...
  assert.match(engineSearch, /push_en_passant_record\(&mut record, board\.en_passant\)/);
...
test("GPU turn completion only spends moves on pending present boards", async () => {
  ...
  assert.match(worker, /limit: options\.limit/);
  ...
  assert.match(worker, /await engineGpuTurnCompletionStep\(current, moves\.length, pendingBoards, status\)/);
  assert.match(worker, /await engineGpuTurnCompletionStep\(snapshot, moves\.length, pendingBoards, status, visited\)/);
  assert.match(worker, /engine\.chronofish_gpu_turn_status_json\(ptr, len\)/);
  assert.match(worker, /await engineFullSearchPrecondition\(turnStatus\)/);
  ...
  assert.match(worker, /await engineNonPostableResultSummary\(gpuResult\)/);
  assert.match(worker, /engine\.chronofish_gpu_non_postable_result_summary_json\(ptr, len\)/);
  assert.match(worker, /gpuResult && await enginePostableSearchResult\(gpuResult\)/);
  assert.match(worker, /engine\.chronofish_gpu_postable_search_result_json\(ptr, len\)/);
  assert.match(worker, /return await engineGpuSearchFailureSummary\(snapshot\)/);
  assert.match(worker, /engine\.chronofish_gpu_search_failure_summary_json\(ptr, len\)/);
  assert.match(worker, /await engineWithCompletedTurnChoice\(result, result\.moves, result\.gpuSearch\)/);
...
test("GPU pending-board filters use engine numeric color normalization", async () => {
  ...
  assert.match(snapshot, /engine\.chronofish_gpu_search_color_code_json\(input\.ptr, input\.len\)/);
