...
test("GPU turn completion only spends moves on pending present boards", async () => {
  ...
  assert.match(worker, /engine\.chronofish_gpu_ranked_candidates_json_bytes\(ptr, byteLength\)/);
  ...
  assert.match(worker, /engine\.chronofish_gpu_supported_mutation_candidate_indexes_bytes\(ptr, byteLength\)/);
